...
% Otherwise statistical techniques may perform better. Another factor we have to take into account is how fast can the system evolve in an ever changing threat landscape and how hard it is to redeploy such a system. Classification, clustering and statistical methods are hard to train but after that testing and deploying is easier. Lastly another cause for potential problems is that in some cases anomalous events are not rare compared to normal ones. In such cases our trained model can end up with high false positive rate. The key take from all these
%points is that an IDS method’s efficiency can be quite dependant on the particular system and environment
%implementation.
...
\bibitem{intro-ids1} Iti Raghav, Shashi Chhikara, Nitasha Hasteer \emph{Intrusion Detection and Prevention in Cloud Environment: A Systematic Review}, International Journal of Computer Applications (0975 – 8887, Volume 68– No.24, April 2013
