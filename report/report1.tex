...
%% supervisor:
% - Good breadth of vocerage. I would like to see more quantitative info.
...
%% supervisor:
% - The flow here is more like a diary than a technical document.
%  !
...
%%supervisor:
% - Repeating to close to each other?
...
\end{table}
%% supervisor:
% - Do this many decimal components make sense?
% > Reduced to 4
% - Would a pie chart look nice?
% > Not because of components with small percentage.
...
Moving forward we want to see the performance of Support Vector Machines on the dataset as a whole instead of training for each attack specifically. Hence we will treat the problem as a two class classification problem with our classes being normal behaviour and attack behaviour. To get a better idea of the performance of our classifier in this case we will use cross validation. To address the issue of class imbalance in our dataset we will use stratified 8-fold cross validation. Going even further we run our simulation for various values of the regularisation parameter. We present our results in table \ref{svmt3}. We can see that we get better performance\footnote{Increased performance means higher difference between precision and fallout rates, with 1 been perfect classification results.} for low regularisation values. The optimum value is $C=0.5$.
...
\end{tabular}
\vspace{5pt}
\caption{8-fold stratified cross validation results for various regularisation parameter values of SVM classifier with linear kernel. Results are presented with mean and standard deviation for two metrics. True Positive rate (Precision) and False Positive rate (Fall out).}
\label{svmt3}
\end{table}
...
\subsection{Linear SVM on the complete frequency feature space} \mbox{}

Moving further away from the approach of Xie et al.\cite{adf1} we investigate the application of support vector machines in the frequency feature space without reducing it's dimensionality through principal component analysis. 
